function [net,stats] = deblur_train_dag(net, varargin)
...
for epoch=start+1:opts.numEpochs
    ...
    if  mod(epoch,100)~=1 && isfield(imdb,'set') ~= 0
        
    else
        clear imdb;
        [imdb] = generatepatches;
%         figure,imshow([imdb.LRlabels(:,:,:,1),imdb.HRlabels(:,:,1,1)],[])
    end
...
for t=1:params.batchSize:numel(subset)
    ...
    %%%--------------------------------------------------
    lossL2 = squeeze(gather(net.vars(net.getVarIndex(params.derOutputs{1})).value));
    %lossL2_3 = squeeze(gather(net.vars(net.getVarIndex(params.derOutputs{3})).value));
        ...
        case  'Adam'
            ...
            switch net.params(p).trainMethod
                ...
                case 'average' % mainly for batch normalization
                    ...
                    % net.params(p).value = smallClipping3(net.params(p).value,20,-20);
                otherwise
                   ...
                   % theta = params.thetaCurrent/lr;
                    net.params(p).value = net.params(p).value - lr * state.m{p} ./ (sqrt(state.v{p}) + params.epsilon);
                    
%                     if mod(count,200)==0
%                         % %
%                         if numel(net.params(p).value) < 300
%                             net.params(p).value = smallClipping(net.params(p).value,1.5);
%                         end
%                         A = net.params(p).value;
%                         if numel(A)>=3*3*96
...
%                                 [U,S,V] = svd(A,'econ');
%                             end
%                             S1 =smallClipping2(diag(S),1.2,0.8);
%                             % S1 =ones(size(diag(S)));
%                             A = U*diag(S1)*V';
%                             A = reshape(A,size(net.params(p).value));
%                             net.params(p).value = A;
%                         end
%                     end
...
inputs = imdb.LRlabels(:,:,1,batch);
labels = imdb.HRlabels(:,:,:,batch);
% kernel = imdb.kernelsum(:,batch);
% sigma  = imdb.sigmasum(:,batch);

% map1 = bsxfun(@times,ones(size(inputs,1)/8,size(inputs,2)/8,1,size(inputs,4)),permute(kernel,[3 4 1 2]));
% map1 = vl_nnSubP(map1,[],'scale',8);
% 
% map2  = bsxfun(@times,ones(size(inputs,1),size(inputs,2),1,size(inputs,4)),permute(sigma,[3 4 1 2]));
% 
% map = cat(3,map1,map2);
